...
struct TestFeedbridgeStandard {
    ...
    func testAddAndRetrieveBaby() async throws {
        ...
        guard let newlyAddedBaby = babies.first(where: { $0.name == testBaby.name }),
              let newlyAddedID = newlyAddedBaby.id else {
    ...
    func testAddWeightEntryToBaby() async throws {
        ...
        let babies = try await standard.getBabies()
        guard let newlyAddedBaby = babies.first(where: { $0.name == testBaby.name }),
              let babyId = newlyAddedBaby.id else {
    ...
    func testAddAndDeleteWetDiaperEntry() async throws {
        ...
        let diaperCountAfterDelete = babyAfterDiaperRemoval?.wetDiaperEntries.wetDiaperEntries.count ?? 0
        #expect(Bool(diaperCountAfterDelete == 0), "Expected wet diaper entry to be deleted from Firestore.")
    ...
    func testAddAndDeleteDehydrationCheck() async throws {
        ...
        let check = DehydrationCheck(dateTime: Date(), poorSkinElasticity: true, dryMucousMembranes: false)
        ...
        let checkCountAfterDelete = babyAfterCheckRemoval?.dehydrationChecks.dehydrationChecks.count ?? 0
        #expect(Bool(checkCountAfterDelete == 0), "Expected dehydration check to be deleted from Firestore.")
